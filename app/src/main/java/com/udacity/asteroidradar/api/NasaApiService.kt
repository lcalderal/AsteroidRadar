...
enum class AsteroidApiFilter(val value: String) {
    WEEK("week"), TODAY("today"), SAVED("saved")
}
...
private val retrofit_moshi = Retrofit.Builder().addConverterFactory(MoshiConverterFactory.create(moshi)).baseUrl(Constants.BASE_URL).build()
